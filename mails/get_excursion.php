	...
	if($_POST){
		...
		if( $Return->success == true && $Return->score > .25 ) {
			...
			if ( $_POST['phone'] ) {
				...
				// Сообщение в Телегу
				$messageInTelegram = "Заявка на экскурсию.%0A";
				$messageInTelegram .= "Телефон: " . $phone;
				
				// Если поле с телефоно заполненно
				mail( "info@yesenin-group.ru, new-lid@yandex.ru", "Заявка на экскурсию на объект с сайта esenin-group.ru", $message, $headers );
