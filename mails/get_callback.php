	...
	if($_POST){
		...
		if( $Return->success == true && $Return->score > .25 ) {
			...
			if ( $_POST['phone'] ) {
				...
				// Сообщение в Телегу
				$messageInTelegram = "Заявка на обратный звонок.%0A";
				$messageInTelegram .= "Клиент: " . $name ."%0A";
				$messageInTelegram .= "Телефон: " . $phone;
				
				// Если поле с телефоно заполненно
				mail( "info@yesenin-group.ru, new-lid@yandex.ru", "Заявка на обратный звонок с сайта esenin-group.ru", $message, $headers );
				...
				$_SESSION['win'] = 1;
				$_SESSION['recaptcha'] = '<p class="text-light">Спасибо за обращение в строительную компанию «Есенин-групп». Мы свяжемся с&#160;Вами в&#160;ближайшее время.</p>';
