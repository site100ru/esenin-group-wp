...
/**
 * Изменяем стандартный loop start для grid
 */
add_filter('woocommerce_product_loop_start', 'mytheme_product_loop_start');
function mytheme_product_loop_start($html)
{
...
/**
 * Изменяем стандартный loop end
 */
add_filter('woocommerce_product_loop_end', 'mytheme_product_loop_end');
function mytheme_product_loop_end($html)
{
    return '</div>';
}
...
/**
 * Изменяем текст опций сортировки на русский
 */
...
/**
 * Убираем стандартный вывод result count и ordering
 */
...
/**
 * Обработка кастомной сортировки
 */
...
/*** END ФУНКЦИИ ДЛЯ АРХИВА ТОВАРОВ ***/
...
/**
 * Изменяем заголовок виджета фильтра по атрибуту
 * Вместо "Filter by" показываем название атрибута
 */
...
{
    // Только для виджетов фильтров в нашей области
...
/*** END РЕГИСТРАЦИЯ ОБЛАСТЕЙ ВИДЖЕТОВ ***/


/*** ИНТЕГРАЦИЯ FIBOSEARCH ***/

/**
 * Добавляем кастомные классы к полю поиска FiboSearch
 */
...
/**
 * Кастомная иконка поиска для FiboSearch
 */
...
add_action('init', function () {
    if (
        ...
        !isset($_GET['delete_all_products']) ||
        $_GET['delete_all_products'] != 1
    ) {
        return;
    }
    ...
    // 2. Удаляем все категории товаров
    $terms = get_terms([
        'taxonomy'   => 'product_cat',
        'hide_empty' => false,
    ]);
    ...
    wp_die('✅ WooCommerce полностью очищен');
});
