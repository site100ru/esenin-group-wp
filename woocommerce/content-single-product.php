        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="product-content">
                    ...
                    $add_amount_units_json = get_post_meta(get_the_ID(), '_gl_add_amount_units', true);

                    $quantity_unit    = $quantity_unit_json    ? json_decode($quantity_unit_json,    true) : null;
                    $add_amount_units = $add_amount_units_json ? json_decode($add_amount_units_json, true) : [];
                    ...
                    // Базовая единица из quantity_unit
                    $base_unit = isset($quantity_unit['name']) ? $quantity_unit['name'] : 'шт';

                    // Список единиц для селекта
                    $units_list = [];

                    // Базовая единица всегда первая, coefficient = 1
                    $units_list[] = [
                        'name'        => $base_unit,
                        'coefficient' => 1,
                        'type'        => 'base',
                    ...
                    // Доп. единицы — только с coefficient > 1
                    if (! empty($add_amount_units) && is_array($add_amount_units)) {
                        foreach ($add_amount_units as $unit) {
                            if (! empty($unit['name']) && isset($unit['coefficient']) && floatval($unit['coefficient']) > 1) {
                                $units_list[] = [
                                    'name'        => $unit['name'],
                    ...
                    // Fallback — если API данных нет совсем
                                    ...
                                    <?php foreach ($units_list as $unit) : ?>
                                        <option
                                            ...
                                            if ($unit['coefficient'] != 1) {
                                                echo ' (×' . esc_html($unit['coefficient']) . ')';
                                            }
                                            ?>
                                        </option>
                                    <?php endforeach; ?>
                        ...
                        <div class="calculator-result">
                            ...
                            <div class="calculator-result-item">
                                ...
                                <div class="calculator-result-value calc-total-price">
                                    <?php echo $has_price ? number_format($product_price, 0, ',', ' ') . ' ₽' : 'Уточняйте'; ?>
                                </div>
                            </div>
                        </div>

                        <div class="calculator-actions">
                            <button class="btn btn-corporate-color-1 product-card__btn calc-add-to-list"
                                type="button"
